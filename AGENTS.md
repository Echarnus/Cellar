...
3. **Tests** — `sh Scripts/test.sh` passes. Unit tests plus the hermetic integration tier; fast,
   no network, and it cannot touch a real bottle. A change to a profile, a store, a launch route or
   any player-visible wording is expected to be *covered* here, not merely to leave it green.
